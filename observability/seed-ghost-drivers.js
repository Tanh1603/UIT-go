 ...
 *
 * Usage (from observability folder):
 *   node seed-ghost-drivers.js
 *   node seed-ghost-drivers.js --count=50000
 *   node seed-ghost-drivers.js --clear (removes all ghost drivers)
 *   node seed-ghost-drivers.js --stats (show current stats)
 */
...
const LOCATION_CONFIG = {
  ...
  // CRITICAL: Must match performance-test.js CENTRAL_HCMC location for cluster bomb test
};
...
async function showStats(client) {
  try {
    ...
    // Sample some ghost drivers
    if (ghostDrivers.length > 0) {
      const sampleCount = Math.min(5, ghostDrivers.length);
      console.log(`Sample ghost drivers (first ${sampleCount}):`);
      for (let i = 0; i < sampleCount; i++) {
        const pos = await client.geoPos('drivers', ghostDrivers[i]);
        if (pos && pos[0]) {
          // Convert to number as geoPos returns strings
          const lng = parseFloat(pos[0].longitude);
          const lat = parseFloat(pos[0].latitude);
          console.log(
            `  - ${ghostDrivers[i]}: [${lng.toFixed(4)}, ${lat.toFixed(4)}]`
          );
        }
      }
      console.log('');
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Ghost Driver Seeder - Populate Redis for performance testing

Usage:
  node seed-ghost-drivers.js                      # Seed 100,000 ghost drivers (default)
  node seed-ghost-drivers.js --count=50000        # Seed 50,000 ghost drivers
  node seed-ghost-drivers.js --clear              # Remove all ghost drivers
  node seed-ghost-drivers.js --stats              # Show current Redis stats
  node seed-ghost-drivers.js --help               # Show this help
...
Examples:
  # Initial setup
  node seed-ghost-drivers.js --count=100000

  # Clean up after failed test
  node seed-ghost-drivers.js --clear

  # Re-run performance test
  node seed-ghost-drivers.js --count=100000
  cd .. && node load-tests/performance-test.js
`);
    process.exit(0);
  }
  ...
  if (
    !shouldClear &&
    !shouldShowStats &&
    (GHOST_COUNT < 1 || GHOST_COUNT > 1000000)
  ) {
